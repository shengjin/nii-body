...
void ichain_mirror_jchain(double *ptr_ichain, double *ptr_jchain, int N_parm);
...
{    
    ...
    if( my_rank == root_rank )
    {
        ...
        if (do_swap)
        {
	    if (running_Beta_Values[j_swap]!=running_Beta_Values[i_swap])
	    {
...
//
//////////////////////////
// let ichain_parameter be the same with jchain
void ichain_mirror_jchain(double *ptr_ichain, double *ptr_jchain, int N_parm)
{
    //
    for (int i=0; i<N_parm; i++)
