...
double zero_stretch;     // how to adjust the ladder when swap acceptance ratio = 0
                 //
...
void read_sampling_para(char *path)
{
    ...
    //
    para_name = "SameBetaCopy";
    para_line = read_onepara(path, para_name);
    sscanf(para_line, "%[^:]:%d", dummy, &SameBetaCopy);
    //
    para_name = "TuneAndJump";
    para_line = read_onepara(path, para_name);
    sscanf(para_line, "%[^:]:%d", dummy, &TuneAndJump);
    //
    free(para_line);
    para_line = NULL;
}
