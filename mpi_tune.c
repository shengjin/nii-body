...
double iter_batch_mh_tune(double **chain_IterParm, double *ptr_sigma_prop, int n_iter_in_tune, int nline_data, double *data_NlineNdim, int rank_in_tune, unsigned *ptr_accumul_accept_tune, double *ptr_logpost_old, int my_rank, double *running_Beta_Values);
...
// LOOP A is a wrapper of this function (i.e., for all ranks needed to be tune 
    ...
    //  
    //  for root rank: store the best position in all the tuning runs
    double **bestPosition_Logpost_NParmTuned;
    bestPosition_Logpost_NParmTuned = alloc_2d_double(N_parm, N_parm+1);
    //
	...
	// TODO: update chain_single_tune
        mpi_tune_sigma_iparmNrank(status, my_rank, n_ranks, root_rank, slavereturn_tag, chain_single_tune, logpost_single_tune, nline_data, data_NlineNdim, ptr_sigma_prop, n_iter_in_tune, ar_ParmNvaried, rank_in_tune, j_parm, running_Beta_Values, bestPosition_Logpost_NParmTuned);
        MPI_Barrier(MPI_COMM_WORLD);
        //
        //////////////////////
        if (my_rank == root_rank)
	...
	if (TuneAndJump)
	{
	    ...
	    {
                for(int j = 0; j < N_parm+1; j++)
 	        {
                    printf("%lf ", bestPosition_Logpost_NParmTuned[i][j]);
	        }
                printf(" %d \n", i);
            }
	}
	*/
	...
	bestPosition_i = 0;
	double bestLogpost;
	bestLogpost = bestPosition_Logpost_NParmTuned[0][N_parm];
	if (TuneAndJump)
        {
            for(int i = 1; i < N_parm; i++)
	    {
	        if (bestLogpost < bestPosition_Logpost_NParmTuned[i][N_parm])
	        {
	            bestPosition_i = i;
	            bestLogpost = bestPosition_Logpost_NParmTuned[i][N_parm];
	        }
	    }
	}
        /*
 	...
 	{
            printf("%lf ", bestPosition_Logpost_NParmTuned[bestPosition_i][i]);
	}
        printf(" %d \n", bestPosition_i);
	*/
	//
	if (TuneAndJump)
	{
	    // update chain
            ...
            for (int i=0; i<N_parm; i++)
            {
                transit_BetaParm_root[rank_in_tune][i] = bestPosition_Logpost_NParmTuned[bestPosition_i][i];
            }
            logpost_all_ranks[rank_in_tune] = bestPosition_Logpost_NParmTuned[bestPosition_i][N_parm];
	    // we haven't find a way to update counting yet.
	}
	//
    }
    //
    free_2d_double(bestPosition_Logpost_NParmTuned);
    bestPosition_Logpost_NParmTuned = NULL;
...
{    
    ...
    */
    ...
    {
    // Work for root_rank
        double **bestPosition_Logpost_NrankTuned;
        bestPosition_Logpost_NrankTuned = alloc_2d_double(n_ranks, N_parm+1);
        //
        //
        ...
        for(int i = 0; i < N_parm; i++)
	{
        ...
        double *ptr_logpost_old;
        ptr_logpost_old = &logpost_old;
        //printf("1st %lf root %d\n", *ptr_logpost_old, root_rank);
        //
        ...
        ar_ParmNvaried[j_parm][root_rank] = iter_batch_mh_tune(chain_IterParm, ptr_sigma_prop, n_iter_in_tune, nline_data, data_NlineNdim, rank_in_tune, ptr_accumul_accept_tune, ptr_logpost_old, my_rank, running_Beta_Values);
	// TODO chain_IterParm[last chain], logpost_old?
	if (TuneAndJump)
	{
            for(int i = 0; i < N_parm; i++)
	    {
                bestPosition_Logpost_NrankTuned[root_rank][i] = chain_IterParm[n_iter_in_tune-1][i];
	    }
            bestPosition_Logpost_NrankTuned[root_rank][N_parm] = *ptr_logpost_old;
        }
        // ...copy the logpost_old for root
	//
        for(int i_rank = 0; i_rank < n_ranks; i_rank++)
        {
            if (i_rank != root_rank)
            {
                // wrong comment?: Collect (ar not logpost_all_ranks[i_rank]s) from slaves
                MPI_Recv(&ar_ParmNvaried[j_parm][i_rank], 1, MPI_DOUBLE, i_rank, slavereturn_tag, MPI_COMM_WORLD, &status);
	        if (TuneAndJump)
	        {
                    MPI_Recv(&bestPosition_Logpost_NrankTuned[i_rank][N_parm], 1, MPI_DOUBLE, i_rank, slavereturn_tag+1, MPI_COMM_WORLD, &status);
                    // Send the final chain in tune
                    MPI_Recv(&bestPosition_Logpost_NrankTuned[i_rank][0], N_parm, MPI_DOUBLE, i_rank, slavereturn_tag+2, MPI_COMM_WORLD, &status);
		}
            }
        }
        /*
            for(int i = 0; i < N_parm; i++)
 	    {
                printf("%lf ", chain_IterParm[n_iter_in_tune-1][i]);
	    }
            printf(" %d \n", root_rank);
        */
        ...
        for(int j = 0; j < n_ranks; j++)
	{
            for(int i = 0; i < N_parm+1; i++)
 	    {
                printf("%lf ", bestPosition_Logpost_NrankTuned[j][i]);
	    }
            printf(" %d \n", j);
	}
	*/
	//
	int bestPosition_rank;
	bestPosition_rank = 0;
	double bestLogpost;
	bestLogpost = bestPosition_Logpost_NrankTuned[0][N_parm];
	if (TuneAndJump)
        {
	    for(int i = 1; i < n_ranks; i++)
	    {
	        if (bestLogpost < bestPosition_Logpost_NrankTuned[i][N_parm])
	        {
	            bestPosition_rank = i;
	            bestLogpost = bestPosition_Logpost_NrankTuned[i][N_parm];
	        }
            }
	}
        //
	/* print test
        for(int i = 0; i < N_parm+1; i++)
 	{
            printf("%lf ", bestPosition_Logpost_NrankTuned[bestPosition_rank][i]);
	}
        printf(" %d \n", bestPosition_rank);
	*/
	//
	if (TuneAndJump)
	{
            for(int i = 0; i < N_parm+1; i++)
 	    {
                bestPosition_Logpost_NParmTuned[j_parm][i] = bestPosition_Logpost_NrankTuned[bestPosition_rank][i];
	    }
	}
        //
        free_2d_double(bestPosition_Logpost_NrankTuned);
        bestPosition_Logpost_NrankTuned = NULL;
	//
    } 
    else  
    {
        ...
        for(int i = 0; i < N_parm; i++)
	{
        ...
        double *ptr_logpost_old;
        ptr_logpost_old = &logpost_old;
        //printf("1st %lf slave %d\n", *ptr_logpost_old, my_rank);
        //
        // accumulate N_accept for the tuning part
        unsigned i_accumul_accept_tune; // updated in iter_batch_mh_tune
        i_accumul_accept_tune = 0; 
        ...
        //
        // Iter the chains
        ar = iter_batch_mh_tune(chain_IterParm, ptr_sigma_prop, n_iter_in_tune, nline_data, data_NlineNdim, rank_in_tune, ptr_accumul_accept_tune, ptr_logpost_old, my_rank, running_Beta_Values);
        //
	...
	if (TuneAndJump)
	{
            // Send the final logpost 
            MPI_Send(ptr_logpost_old, 1, MPI_DOUBLE, root_rank, slavereturn_tag+1, MPI_COMM_WORLD); 
            // Send the final chain in tune
            MPI_Send(&chain_IterParm[n_iter_in_tune-1][0], N_parm, MPI_DOUBLE, root_rank, slavereturn_tag+2, MPI_COMM_WORLD);
	}
        //
        // Free temporary memory alloc.ed
        free_2d_double(chain_IterParm);
...
double iter_batch_mh_tune(double **chain_IterParm, double *ptr_sigma_prop, int n_iter_in_tune, int nline_data, double *data_NlineNdim, int rank_in_tune, unsigned *ptr_accumul_accept_tune, double *ptr_logpost_old, int my_rank, double *running_Beta_Values)
{
    ...
    *(logpost+0) = *ptr_logpost_old;
	...
	// ...check and bounce the parms in range
        ...
        //
        //
        if ( (logpost_new - *ptr_logpost_old) > log(1/q_factor) )
        {
            for (int j=0; j<N_parm; j++)
            {
        ...
        {
            H = exp(logpost_new - *ptr_logpost_old) * q_factor;
            rand_unif = drand48( );
            if (rand_unif < H)
            {
                for (int j=0; j<N_parm; j++)
        ...
        if ( (copy_old) && (i>0) )
        {
            ...
            *(logpost+i) = *ptr_logpost_old;  // 3rd setting, total 3
        }
        // 
        //
        ///////////////////////////////
    ...
    // 
    // used in TuneAndJump
    *ptr_logpost_old = logpost[n_iter_in_tune-1];
    //
